...
#define _DIST_ALPHA 0.0   //[3054] 센서 보정정도
          // [3131] EMA필터값
// Servo range
#define _DUTY_MIN 1220    // [3052] 서보의 최소각도를 microseconds로 표현
#define _DUTY_NEU 1470    // [3070] 레일플레이트 중립위치를 만드는 서보 duty값 1465
#define _DUTY_MAX 1625                //[3062] 서보의 최대각도의 microseconds의 값

// Servo speed control
#define _SERVO_ANGLE 10   // [3131] 최대 가동범위에 따른 목표 서보 회전각
#define _SERVO_SPEED 500   //[3064]서보 속도를 30으로
...
// PID parameters
#define _KP 1.0  // [2635] P 이득 비율(?)
#define _KI 0.01   // [2635] I 이득 비율
#define _KD 7.5 // [2635] D 이득 비율

#define _INTERVAL_DIST_F 30  // DELAY_MICROS * samples_num^2 의 값이 최종 거리측정 인터벌임. 넉넉하게 30ms 잡음.
...
error_curr = _DUTY_NEU;
error_prev = _DUTY_NEU;
spark = 0;
// move servo to neutral position
  myservo.writeMicroseconds(_DUTY_NEU); //[2635]

// initialize serial port
  Serial.begin(115200); //[2635]
...
void loop() {
    ...
    if(event_dist) {
        ...
        dist_raw = distR(filtered_ir_distance()); // [2635] 센서 값 받아와서 변수에 저장
        
        // PID control logic
        error_curr = _DIST_TARGET - dist_raw; // [2635] 오차 계산

        int errm = 1;
        if (error_curr<5&&error_curr>-10){
          errm = 0.9;
        }

        iterm += _KI*error_curr;
        
        dterm = dema(_KD * (error_curr - error_prev));
        error_prev = error_curr;
        
        pterm = pterm_cal(error_curr); // [2635] p값은 오차
        control = _KP * pterm + _KI * iterm +  _KD * dterm; // [2635] control 값, i와 d는 현재 0

        // duty_target = f(duty_neutral, control)
        duty_target = _DUTY_NEU + control; // [3070] control 값이 다 합해서 1이 되도록 되어있다면, 중립 위치에 컨트롤 값 만큼의 비율을 더해 목표위치를 정한다.
        //duyy_target = _DUTY_NEU + (control - X) * (control > X ? _DUTY_MAX - _DUTY_NEU : _DUTY_NEU - _DUTY_MIN); // [2635] NEU 에다가 위아래로 control 에 맞게 조절한 값?
  ...
  // keep duty_target value within the range of [_DUTY_MIN, _DUTY_MAX]
    ...
    if(event_servo) {
        ...
        myservo.writeMicroseconds((int)duty_curr); // [3070](int)duty_curr
    ...
    if(event_serial) {
        ...
        Serial.print("IR:");
        Serial.print(dist_raw);//filtered_ir_distance()
        Serial.print(",T:");
...
float pterm_cal(float error){
  if (error < 50 || error > -50){
    error *=0.95;
  }
  if (error < 0){
    error = error*(_DUTY_NEU-_DUTY_MIN)/155;
  }
  else{
    error = error*(_DUTY_MAX-_DUTY_NEU)/155;
  }
  return error;
}

float distR (float raw){
  float dis;
  if (raw>193){
    dis = map(raw, 193, 234, 305, 410);
  }
  else if (raw <= 193 || raw > 176){
    dis = map(raw, 176, 193, 255, 305);
  }
  else if (raw <= 176 || raw >142){
    dis = map(raw, 142, 176, 205, 255);
  }
  else{
    dis = map(raw, 76, 142, 100, 205);
  }
  if (spark==0){
    dis = 255;
    spark+=1;
