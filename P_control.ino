...
#define _DIST_ALPHA 0.0   //[3054] 센서 보정정도
          // [3131] EMA필터값
// Servo range
#define _DUTY_MIN 1405    // [3052] 서보의 최소각도를 microseconds로 표현
#define _DUTY_NEU 1635    // [3070] 레일플레이트 중립위치를 만드는 서보 duty값
#define _DUTY_MAX 1845                //[3062] 서보의 최대각도의 microseconds의 값

// Servo speed control
#define _SERVO_ANGLE 35   // [3131] 최대 가동범위에 따른 목표 서보 회전각
#define _SERVO_SPEED 45   //[3064]서보 속도를 30으로
...
void loop() {
    ...
    if(event_dist) {
        ...
        dist_raw = map(filtered_ir_distance(),74,254,100,410); // [2635] 센서 값 받아와서 변수에 저장
...
float pterm_cal(float error){
  if (error < 0){
    error = error/155*(_DUTY_NEU-_DUTY_MIN)/_DUTY_NEU;
  }
  else{
    error = error/155*(_DUTY_MAX-_DUTY_NEU)/_DUTY_NEU;
  }
  return error;
}
