...
#define _DIST_ALPHA 0.0   //[3054] 센서 보정정도
          // [3131] EMA필터값
// Servo range
#define _DUTY_MIN 1225    // [3052] 서보의 최소각도를 microseconds로 표현
#define _DUTY_NEU 1465    // [3070] 레일플레이트 중립위치를 만드는 서보 duty값 1465
...
void loop() {
  ...
  // keep duty_target value within the range of [_DUTY_MIN, _DUTY_MAX]
    ...
    Serial.print(",pterm:");
    Serial.print(map(error_curr,-1000,1000,510,610));
    Serial.print(",dterm:");
    Serial.print(map(dterm,-100,100,510,610));
...
float distR (float raw){
  float dis;
  if (raw>173){
    dis = map(raw, 173, 231, 255, 410);
  }
  else{
    dis = map(raw, 67, 173, 100, 255);
  }
  return dis;
}
